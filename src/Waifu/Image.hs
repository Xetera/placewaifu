...
fromByteString bs =
  ...
  where
    ...
    metadata =
      either
        (const $ error "could not decode image")
        snd
        (P.decodeImageWithMetadata bs)
    getMeta :: P.Keys a -> a
    getMeta =
      maybe (error "could not read metadata") id . flip P.lookup metadata

resize :: (Word, Word) -> Image -> Image
resize size img = img {imgResize = size}

aspectRatio :: Image -> Float
aspectRatio Image {imgSize = (x, y)} = fromIntegral x / fromIntegral y
